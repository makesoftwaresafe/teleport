...
func (u *HostUserManagement) startSemaphoreLoop() error {
	var err error
	u.lease, err = services.AcquireSemaphoreWithRetry(u.ctx,
		services.AcquireSemaphoreWithRetryConfig{
			Service: u.storage,
			Request: types.AcquireSemaphoreRequest{
				SemaphoreKind: types.SemaphoreKindHostUserCleanup,
				SemaphoreName: "user-cleanup-sem",
				MaxLeases:     1,
	...
	go func() {
		semTicker := time.NewTicker(time.Second * 15)
		defer semTicker.Stop()
		for {
			select {
			case <-semTicker.C:
				if err := u.storage.KeepAliveSemaphoreLease(u.ctx, *u.lease); err != nil {
					log.Debug("Failed to keep alive semaphore lease")
				}
			case <-u.ctx.Done():
				return
			}
...
func (u *HostUserManagement) UserCleanup() {
	if err := u.startSemaphoreLoop(); err != nil {
		log.Error("Failed to acquire semaphore: ", err)
		return
	}
